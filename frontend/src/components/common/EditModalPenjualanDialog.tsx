...
type EditModalPenjualanDialogProps = {
  open: boolean;
...
export default function EditModalPenjualanDialog({
  open,
  productCost,
  batches,
  onClose,
  onSuccess,
}: EditModalPenjualanDialogProps) {
  /* =======================================
     FORM STATE
  ======================================== */
  ...
  const selectedBatch = useMemo(() => {
    ...
    return batches.find(
      (batch) => batch.id === productCost.batch_id,
    );
  }, [batches, productCost]);
  ...
  useEffect(() => {
    ...
    setQty(String(productCost.qty));
    setModalBeli(String(productCost.total_modal));
    setTransactionDate(
      productCost.transaction_date || getTodayDate(),
    );
    setSelectedBankAccountId(
  ...
  const handleSubmit = async (
    ...
    if (!productCost) {
      setError("Data modal penjualan tidak ditemukan.");
      return;
    }
    ...
    try {
      ...
      await updateProductCost(productCost.id, {
        total_modal: modalBeliNumber,
        qty: qtyNumber,
      ...
      <div className="w-full max-w-[600px] overflow-visible rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-start justify-between border-b border-[#e7ebf3] px-6 py-5">
          ...
          <div>
            ...
            <h2 className="text-xl font-bold text-[#10245c]">
              Edit Modal Penjualan
            </h2>

            <p className="mt-1 text-sm text-[#7a89ad]">
              Perbarui data modal untuk
              batch penjualan.
            </p>
        ...
        >
          ...
          <div>
            ...
            <div className="mt-2 flex min-h-11 w-full items-center rounded-lg border border-[#d8dfec] bg-[#f7f8fb] px-3 text-sm text-[#20366f]">
              {selectedBatch ? (
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="shrink-0 text-xs text-[#7a89ad]">
                    {countryConfig[selectedBatch.country].name}
                  </span>
                </div>
              ) : (
